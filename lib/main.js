...
module.exports = (Analysis, options) => {
  options = {
    "port":                    "node-port"    in options ? options["node-port"]    : 8000,
  ...
  const child = ChildProcess.fork(Path.join(__dirname, "orchestrator-bin.js"), [semaphore, JSON.stringify(options)]);
  while (true) {
    try {
